class LoginsController < ApplicationController
  ...
  # GET /logins/1.json
  def show
  @user = Login.where(:email => params[:email],:password => params[:password])
  logger.debug "user: #{@user}"
  if @user
       format.html { redirect_to @users.show , notice: "Login successfully"}
       format.json {render json: @users.show}
   else
        format.html{render action: "new"}
        format.json{ render json: @users.error}
     end
 end
  # link to login.html.erb
  def login
    @login = Login.new
